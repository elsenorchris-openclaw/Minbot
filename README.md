...
| `MAX_EDGE` | `0.55` | Skip edges > 55%. 0.40 → 0.42 (2026-04-27) → 0.55 (2026-04-28 night, paired with NBP-vs-recent-CLI bypass). Bypassed by `_nbp_consistent_with_recent_cli` — when forecast μ is within ±2°F of the station's last-7-day CLI low range, an extreme apparent edge is treated as a stale market quote (cheap-tail T-high BUY_YES like AUS-T71/SATX-T73), not a model error. Outliers (NBP wildly off vs recent reality, e.g. 2026-04-26 KMDW NBP=76 vs CLI 44-51) still hit the cap. |
...
| `MIN_MODEL_PROB` / `MAX_MODEL_PROB` | `0.15` / `0.85` | Skip wildly unlikely or near-certain. **Also bypassed by `_nbp_consistent_with_recent_cli`** (2026-04-28 night) — without that, AUS-T71-style cheap-tail wins (mp 95% / yes_ask 3¢ / 4-of-4 recent CLI ≥ threshold) get blocked here even after MAX_EDGE bypasses. Both gates protect against "model too confident"; both are wrong when recent CLI supports the forecast. |
